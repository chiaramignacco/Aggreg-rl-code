...
# In thiss call we implement the model and the policies
...
class Aggregation: 
    def __init__(self, graph, arrival_rates, experts, eta, queue_max, Q=None):
        ...
        self.advantage_net = AdvantageNetwork(self.state_dim, self.action_dim, self.cLI)
        self.optimizer = optim.AdamW(self.advantage_net.parameters(), lr=0.00001)
    ...
    def train_advantage_network_direct(self, weights, episodes=200, entropy_param=0.1): 
        ...
        for episode in range(episodes):
            ...
            for _ in range(100):
                ...
                with torch.no_grad():
                    q_values = self.advantage_net(state_tensor)
                ...
                next_q_value = torch.max(self.advantage_net(next_state_tensor)[0]) #[action_next]
                
                target = reward + 0.9 * next_q_value # - entropy_param * np.log(self.weights[next_state, action_next]))
                
                # clipped_target = torch.clamp(target, min=-self.cLI, max=self.cLI + 200)
                ...
                #####
                # for param in self.advantage_net.parameters():
        ...
        with torch.no_grad():
            output = self.advantage_net(state_space_tensor)  # Shape: (num_states, m)
    ...
    def train_advantage_network(self, weights, episodes=500, entropy_param=0.1): 
        ...
        for episode in range(episodes):
            ...
            for _ in range(200):
                ...
                next_q_value = self.advantage_net(next_state_tensor)[0][action_next]
                ...
                # requires_grad=True
                loss = self.loss_fn(q_values[0, action], clipped_target)
                self.optimizer.zero_grad()
