...
class QNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=16):
        ...
        self.fc1 = nn.Linear(state_dim, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim)
        self.fc3 = nn.Linear(hidden_dim, action_dim)  # Outputs q values
...
def train_actor_critic(sdm, episodes=10000, learning_rate=0.00001, lr_decay_rate=0.9, discount_factor=0.9):
    state_dim = sdm.num_nodes
    action_dim = len(sdm.experts)
    
    # Initialize policy and critic networks
    policy_net = PolicyNetwork(state_dim, action_dim)
    critic_net = QNetwork(state_dim, action_dim)
    ...
    for episode in tqdm(range(episodes), desc="Training Progress"):
        ...
        for _ in range(1000):  # Interactions within one episode
            ...
            q_values = critic_net(state_tensor)
            
            with torch.no_grad():
                policy_probs_next = policy_net(next_state_tensor)
                
            action_next = torch.multinomial(policy_probs_next, num_samples=1).item()
            
            # Compute TD target for critic
            target = reward + 0.9 * critic_net(next_state_tensor)[0][action_next] 
            
            # Update the critic network
            critic_loss = (q_values[0, action] - target) ** 2  
            critic_optimizer.zero_grad()
            critic_loss.backward()  
            critic_optimizer.step()

            # Update the policy using the advantage values
            q_value = q_values[0].detach() 
            
            # print(q_value, policy_probs, q_value * policy_probs[0])
            V = torch.sum(q_value * policy_probs[0])#.reshape(Q.shape[0], 1)       
            advantage = q_value - V
            # print(advantage, q_value, V)
...
# Training loop
def train_actor_critic_V2(sdm, episodes=1000, learning_rate=0.00005, lr_decay_rate=0.9, discount_factor=0.9):
    state_dim = sdm.num_nodes
    action_dim = len(sdm.experts)
    ...
    for episode in tqdm(range(episodes), desc="Training Progress"):
        ...
        for _ in range(1000):
            # Update the policy using the advantage values
            state_tensor = torch.FloatTensor(state).unsqueeze(0).requires_grad_()  
