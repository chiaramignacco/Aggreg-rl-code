...
#             model.queue = {vertex: 0 for vertex in self.graph}
            ...
            states, states_idx = self.train_double_dqn(self.agent, 20)
            
            states_tensor = torch.tensor(states, dtype=torch.float32).unsqueeze(0) 
            
            curr_weights = self.policy_net(states_tensor)  
            
            scaled_A = 0.1 * self.A[0][states_idx] / (1 - 0.9) # Equivalent to scaling by 0.1
    ...
    def train_double_dqn(self, agent, num_episodes, epsilon_start=1.0, epsilon_end=0.01, epsilon_decay=0.995, target_update_freq=10):
        ...
        for episode in range(num_episodes):
            ...
            for _ in range(100):
                ...
                # action = agent.select_action(state, epsilon, self.policy_net)
                if np.random.rand() < 0.1:
                    action = np.random.randint(self.K)
                else:
                    # e = np.random.choice(np.arange(self.K), p=self.weights[state]) 
                    with torch.no_grad():
                        action = self.policy_net(torch.tensor(curr_state, dtype=torch.float32).unsqueeze(0) ).multinomial(1).item()
                ...
                agent.update(self.policy_net, model)
...
#         with torch.no_grad():
#             output = agent.q_network(state_space_tensor)
#             weights= self.policy_net(state_space_tensor)# Shape: (num_states, m)

        Q = agent.q_network(state_space_tensor)
        weights = self.policy_net(state_space_tensor)# Shape: (num_states, m)
        ...
        # V = (np.sum(Q * weights, axis=1)).reshape(Q.shape[0], 1)    
        
        V = (Q * weights).sum(dim=2)
        # print(Q.shape, V.unsqueeze(-1).shape)
        A = Q - V.unsqueeze(-1)
...
class DoubleDQNAgent:
    def __init__(self, state_dim, action_dim, gamma=0.9, lr=1e-3, batch_size=64, buffer_size=10000):
        ...
        self.optimizer_q = optim.Adam(self.q_network.parameters(), lr=lr)
    ...
    def update(self, weights, model):
        ...
        with torch.no_grad():
            ...
            next_actions = [weights(state).multinomial(1).item() for state in states_tensor]
