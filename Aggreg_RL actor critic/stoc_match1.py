...
class Aggregation: 
    ...
    def train_actor_critic(self, num_episodes, epsilon_start=1.0, epsilon_end=0.01, epsilon_decay=0.995, target_update_freq=10):
        ...
        for episode in range(num_episodes):
            ...
            for _ in range(100):
                ...
                action_probs = self.agent.get_policy(state_tensor) 
                ...
                next_state_tensor = torch.FloatTensor(next_s).unsqueeze(0) 
                
                self.agent.store_transition(curr_state, action, reward_norm, next_s)
    
                state = next_state
                total_reward += reward
    
                self.agent.update(model)
...
#         with torch.no_grad():
#             output = self.agent.q_network(state_space_tensor)  # Shape: (num_states, m)
...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        return x #torch.softmax(self.fc2(x), dim=-1)
...
#         return np.random.choice(np.arange(self.action_dim), p=weights[state]) #torch.argmax(q_values).item()
    ...
    def update(self, model):
        ...
        with torch.no_grad():
            ...
            next_actions = [np.random.choice(self.action_dim, p=np.array(self.get_policy(state))) for state in states_tensor] 
        ...
        target_q_values1 = rewards + self.gamma * next_q_values
            
        action_probs = self.get_policy(states_tensor)  
        
        # next_action_probs = self.actor(next_states_tensor)
        
        value = torch.sum(action_probs * target_q_values1)
        
        advantage = target_q_values1 - value
        # print(advantage)
        
        actor_loss = - advantage  #- torch.log(action_probs[0, actions]) * advantage.detach() # minus??
